<?php
// Check If Server User Coming From HTTP Post Request
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Get Variables From Form
    $username = filterRequest('username');
    $password1 = filterRequest('password1');
    $password2 = filterRequest('password2');
    $active = filterRequest('active');
    // Array Errors
    $errors = array();
    // UserName Error
    if (empty($username)) {
        $errors[] = 'Enter User Name';
    } elseif (getCount('admins', 'username = ? && id != ?', array($username, id())) > 0 || getCount('users', 'username = ? && id != ?', array($username, id())) > 0) {
        $errors[] = 'User Name Is Exists';
    }
    // Password Error
    if (empty($password1)) {
        $errors[] = 'Enter Password';
    } elseif (strlen($password1) < 8) {
        $errors[] = 'Password Cannot Be Less Then 8 Characters';
    } elseif (empty($password2)) {
        $errors[] = 'Reenter Password';
    } elseif ($password1 != $password2) {
        $errors[] = 'The Password Is Not Equal';
    }
    // Edit Informatio To Database
    if (empty($errors)) {
        $edit = $con->prepare('UPDATE admins SET username = ?, pass = ?, active = ? WHERE id = ?');
        $edit->execute(array($username, $password1, $active, id()));
        header('location: ?Page=View');
    }
}
?>
<!------------------------ Content ------------------------>
<div class="content">
    <!------------------------ Navbar ------------------------>
    <nav class="navbar navbar-expand-lg">
        <a class="navbar-brand">
            <span><?php echo $pageTitle ?></span>
        </a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent"
            aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link btn btn-danger btn-sm" aria-current="page" href="?Page=View">
                        <i class="fa fa-chevron-left fa-fw"></i>
                        <span>Back</span>
                    </a>
                </li>
            </ul>
        </div>
    </nav> <!-- Navbar -->
    <!------------------------ Form ------------------------>
    <form method="post" enctype="multipart/form-data">
        <!------------------------ Container ------------------------>
        <div class="container">
            <!------------------------ Beasic ------------------------>
            <div class="panel">
                <!------------------------ Title ------------------------>
                <div class="panel-heading">Edit Admin</div>
                <!------------------------ Body ------------------------>
                <div class="panel-body">
                    <!------------------------ Raw ------------------------>
                    <div class="row">
                        <div class="col-12">
                            <?php
                            if (isset($errors) && !empty($errors)) {
                            foreach ($errors as $error) {
                            ?>
                            <div class="alert alert-danger">
                                <?php echo $error ?>
                            </div>
                            <?php } } ?>
                        </div>
                        <!------------------------ UserName ------------------------>
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <div class="form-group">
                                <label>UserName</label>
                                <input
                                    type="text"
                                    name="username"
                                    class="form-control"
                                    autocomplete="off"
                                    value="<?php if(isset($username)){echo$username;}else{echo getItem('admins', id(), 'id = ?', 'username');}?>">
                            </div>
                        </div> <!-- UserName -->
                        <!------------------------ Password ------------------------>
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <div class="form-group">
                                <label>Password</label>
                                <input
                                    type="password"
                                    name="password1"
                                    class="form-control"
                                    autocomplete="off"
                                    value="<?php if(isset($password1)){echo$password1;}else{echo getItem('admins', id(), 'id = ?', 'pass');}?>">
                            </div>
                        </div> <!-- Password -->
                        <!------------------------ Password ------------------------>
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <div class="form-group">
                                <label>Reenter Password</label>
                                <input
                                    type="password"
                                    name="password2"
                                    class="form-control"
                                    autocomplete="off"
                                    value="<?php if(isset($password2)){echo$password2;}else{echo getItem('admins', id(), 'id = ?', 'pass');}?>">
                            </div>
                        </div> <!-- Password -->
                        <?php if (id() != 1) { ?>
                        <!------------------------ Active ------------------------>
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <div class="form-group">
                                <label>Active</label>
                                <div class="form-group">
                                    <select name="active" class="form-control">
                                    <?php foreach (active() as $k => $v) { ?>
                                        <option value="<?php echo $k?>"
                                        <?php if((isset($active)&&$active==$k)){echo'selected';}elseif(getItem('admins',id(),'id=?','active')==$k){echo'selected';}?>>
                                            <?php echo $v ?>
                                        </option>
                                        <?php } ?>
                                    </select>
                                </div>
                            </div>
                        </div> <!-- Active -->
                        <?php } ?>
                    </div> <!-- Raw -->
                </div> <!-- Body -->
            </div> <!-- Beasic -->
            <div class="form-group">
                <button type="submit" class="btn btn-success btn-sm">Edit</button>
            </div>
        </div> <!-- Container -->
    </form> <!-- Form -->
</div> <!-- Content -->